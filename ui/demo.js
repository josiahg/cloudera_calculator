...
// Distinct array item helper function
const distinct = (value, index, self) => { return self.indexOf(value) == index;}
// end distinct

// Read AWS pricing test CSV
var csvdata;
var transposed;
var regions;
$.ajax({
  ...
  transposed = csvdata[0].map((col, i) => csvdata.map(row => row[i]));
  regions = transposed[1].filter(distinct);
  $.each(regions, function(key, value){
    $("#envRegionSelect").append('<option value="'+value+'">'+value+'</option>');
});
  }   
});
// end testing csv reads

// Class to manage state for environment
class Environment {
  constructor(name, provider, region) {
    this.name = name;
    this.provider = provider;
    this.region = region;
    this.infra = {'FreeIPA Service':['m5.large',1],'IDBroker Service':['m5.2xlarge',1],
        'DataLake Master Node':['m5.2xlarge',1],'RDS DB Instance':['db.m5.large',1]};
    if(provider == 'AWS') {
      this.awscost = 761;
      this.azurecost = 0;
    } else {
  ...
  getCldrCost() {
    return this.cldrcost;
  }

  getTotalCost() {
    return this.awscost + this.azurecost + this.cldrcost;
  }
}

// Class to manage state for an estimate
class Estimate {
  constructor(estimateId) {
    //this.environments = {'demo-aws':['demo-aws','AWS','US East (N Virginia)'], 'demo-azure':['demo-azure','Azure','West Central US']};
    this.environments = {};
    this.dhclusters = [];  
    this.updateEnvList();
    this.updateCosts(); 
  ...
  addEnvironment(name, provider, region) {
    this.environments[name] = new Environment(name, provider, region);
  }
  ...
  updateEnvList() {
    ...
    for(let env in this.environments) {
      var provider = this.environments[env].getProvider();
      var region = this.environments[env].getRegion();
      var name = this.environments[env].getName();
      var icon;
      if(provider == 'AWS') { icon = 'fa-amazon' } else { icon = 'fa-windows' }
      $('#envListTable').append('<tr>     <td class="primary data-name"><a href="#">'+name+'</a></td>    <td class="text-center text-base data-status"><i class="icon fa '+icon+'"><span class="sr-only">'+provider+'</span></td>    <td class="data-username">'+region+'</td>    <td class="data-login">730</td>    <td>      <ul class="actions">        <li>          <a href="#"><i class="icon fa fa-pencil" data-toggle="tooltip" data-placement="right" title="Edit"></i></a>        </li>        <li class="del-can">          <a href="#" onclick="delEnvRow(event, \''+name+'\')" class="del-can"><i class="icon fa fa-trash del-can" data-toggle="tooltip" data-placement="right" title="Delete"></i></a>        </li>      </ul>    </td>    <th scope="row" class="text-center">      <div class="form-check">        <input class="form-check-input table-example-check" type="checkbox" value="1" id="row1">        <label class="form-check-label" for="row1"></label>      </div>    </th>  </tr>');
      //$('#envListTable').append('<tr><td class="primary data-name"><a href="#">'+name+'</a></td><td class="text-center text-base data-status"><i class="icon fa '+ icon + '"><span class="sr-only">Azure</span></td><td class="data-username">'+region+'</td><td class="data-login">730</td><td><ul class="actions"><li><a href="#"><i class="icon fa fa-pencil" data-toggle="tooltip" data-placement="right" title="Edit"></i></a></li><li class="del-can" row-del-id="'+name+'" ><a href="#"><i class="icon fa fa-trash" data-toggle="tooltip" data-placement="right" title="Delete"></i></a></li></ul></td><th scope="row" class="text-center"><div class="form-check"><input class="form-check-input table-example-check" type="checkbox" value="2" id="row2"><label class="form-check-label" for="row2"></label></div></th></tr>')
    }
    this.updateCosts();
  }
  ...
  updateCosts() {
    ...
    for(let env in this.environments) {
      ...
      console.log(env)
      total += this.environments[env].getTotalCost();
      aws += this.environments[env].getAwsCost();
      azure += this.environments[env].getAzureCost();
    }
    $('#envCostTotal').text('$' + total)
    $('#envCostAws').text('$' + aws)
    $('#envCostAzure').text('$' + azure)
  }
...
$('.wizard-finish-button').on('click', function(){
  var provider = $('#envProviderSelect').val();
  var region = $('#envRegionSelect').val();
  var name = $('#envInputName').val();
  demoEstimate.addEnvironment(name, provider, region);
  demoEstimate.updateEnvList();
});

function delEnvRow(e, name) {
  e.preventDefault();
  demoEstimate.delEnv(name);
  demoEstimate.updateEnvList();
};
  ...
  updateDisplayedStep(targetStep) {
    if (this.wizard) {
      ...
      if (targetStep === 2) {
        ...
        console.log($('#envProviderSelect').val());
        console.log($('#envRegionSelect').val());
        console.log($('#envInputName').val());
      }
    }
  }
